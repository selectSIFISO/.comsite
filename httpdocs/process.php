<?php
    ...
    if (count($errors) == 0) {
        ...
        $query = sprintf(
            //"insert into tblalbum (ArtistID,AlbumName,AlbumIMG,Release,LabelID,GenreID,filename,file_size,mime_type)
                //values (1,'Loliwe','%s','01-01-1900',1,7,'%s', %d, '%s')",
                //"insert into images (filename, mime_type, file_size, file_data)
                //values ('%s', '%s', %d, '%s')",
                
                "insert into tblalbum (filename, mime_type, file_size, AlbumIMG, ArtistID, AlbumName, LabelID, GenreID, ReleaseYear)
                values ('%s', '%s', %d, '%s',1,'Loliwe',1,7,2011)",
